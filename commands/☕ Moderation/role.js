...
module.exports = {
    ...
    run: async (client, message, args, db) => {
        ...
        .setColor(ee.color)
        .setDescription(`**Example:** \`<role> <color> <name> <emoji>\` syntax! once finished type \`done\`. (Can use custom emojis)\n\n**COLORS**:\n| Blue | Grey | Red | Blurple | Green |`)
        //.addField("COLORS", `\n・Blue\n・Grey\n・Red\n・Blurple\n・Green`)
        //.addField()
        ...
        collector.on('end', async (msgs, reason) => {
            if (reason == 'DONE') {
                ...
                let text = db.fetch(`text_message_${message.guild.id}`)
                const embed = new Discord.MessageEmbed()
                    .setTitle(titlee)
                    .setDescription(text)
                    .setColor(ee.color)
                    .setTimestamp();
                Nuggies.buttonroles.create({ message, content: embed, role: brmanager, channelID: "847730927198011399" })
            }
        })
    }
}