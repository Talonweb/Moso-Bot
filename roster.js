...
module.exports = client => {
    ...
    async function edit_msg(guild) {
        ...
        let rosterembed = new Discord.MessageEmbed()
            ...
            .setDescription("Here is are current staff hierarchy")
            .setImage("https://cdn.discordapp.com/attachments/663520256899153931/873421123050688532/Staff_Banner_1.png")
        ...
        for (let i = 0; i < rosterroles.length; i++) {
            ...
            rosterembed.addField("**" + role.name.toUpperCase() + "**", role.members.length === 0 ? "No one has this Role" : ">>> " + role.members.map(member => `<@${member.user.id}> `).join("\n").substr(0, leftnum), true)
        }
