    ...
    */
    async present(member) {
        ...
        await handleChannelType(this.client, this.options, user).then(async channel => {
            ...
            try {
                ...
                captchaEmbed = await channel.send({
                    embed: new Discord.MessageEmbed()
                        ...
                        .addField("Crack The Captcha", `${member.user}, to gain access to **${member.guild.name}**, please solve the CAPTCHA below within 60 seconds!\n\nThis is done to protect the server from Spam.`)
                        .setFooter("If You Fail To Solve The Captcha You Will Have To Rejoin.")
                        .setColor("RANDOM")
                        .setThumbnail(member.guild.iconURL())
                        .attachFiles([{ name: "captcha.png", attachment: captcha.image }])
            ...
            await captchaEmbed.channel.awaitMessages(captchaFilter, {
                max: 1, time: 60000
            })
                .then(async responses => {
                    if (!responses.size) {
                        ...
                        return channel.send({ embed: captchaIncorrect })
                            .then(async msg => {
                                if (channel.type === "text") msg.delete({ timeout: 3000 });
                            });
                    }

                    const answer = String(responses.first());
                    if (channel.type === "text") await responses.first().delete();

                    if (answer === captcha.text) {
                        await member.roles.add(this.options.roleID)
                        if (channel.type === "text") await captchaEmbed.delete();
                        return channel.send({ embed: captchaCorrect })
                            .then(async msg => {
                                if (channel.type === "text") await msg.delete({ timeout: 3000 });
                            });
                    } else {
                        await member.kick("Failed to Pass CAPTCHA")
                        if (channel.type === "text") await captchaEmbed.delete();
